...
public class ReaderService {
    ...
    public static void showStatistics() {
        try (Connection connection = getConnection()) {
            ...
            ResultSet countBooks = countBooksStatement
                    .executeQuery("SELECT COUNT(*) AS count FROM books");
            ResultSet countOfAvailableBooks = countOfAvailableBooksStatement
                    .executeQuery("SELECT COUNT(*) AS availableBooks FROM books WHERE available = true");
            ResultSet countOfNonAvailableBooks = countOfNonAvailableBooksStatement
                    .executeQuery("SELECT COUNT(*) AS nonAvailableBooks FROM books WHERE available = false");
            ResultSet activeReaders = activeReadersStatement
                    .executeQuery("SELECT COUNT(reader_id) AS activeBooks FROM readers INNER JOIN loans l on readers.id = l.reader_id WHERE l.return_date IS NULL");
