package kz.practice.comparator.task_1;
...
public class PotatoLaboratory {
    ...
    private static List<Potato> findPotatoesForExperiment(List<Potato> potatoes) {
        ...
        //сортировка картофелин
        Collections.sort(newPotatoes);

        //список для результатов
        List<Potato> results = new ArrayList<>();

        //из отсорторованного списка newPotatoes берем первые две самые маленькие картофелины
        //и кладем их в results
        results.add(newPotatoes.get(0));
        results.add(newPotatoes.get(1));

        //из отсорторованного списка newPotatoes берем последнюю и предпоследнюю картофелины
        //и кладем их в results
        results.add(newPotatoes.get(newPotatoes.size() - 2));
        results.add(newPotatoes.get(newPotatoes.size() - 1));
